...
export default class MovieService {
    ...
    static async getTrendMovies(limit: number, page: number): Promise<IMovieAPIResponse> {
        return await HTTPService.get(`${API_URL}/movie?token=${API_KEY}&field=year&search=2020-${getCurrentYear()}&limit=${limit}&page=${page}&sortField=votes.imdb&sortType=-1&sortField=year&sortType=-1&sortField=rating.kp&sortType=-1&selectFields=genres year name description ageRating id poster rating`)
            .then(responseToJSONHandler)
            .catch(console.error)
    }
