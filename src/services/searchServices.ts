...
export default class SearchServices {
    static async getSearchResults( search: string, limit: number ): Promise<IMovieAPIResponse>  {
        return await HTTPService.get(`${API_URL}/movie?search=${search}&field=name&limit=${limit}&sortField=rating.kp&sortType=-1&isStrict=false&selectFields=genres countries name id poster rating &token=${API_KEY}`)
